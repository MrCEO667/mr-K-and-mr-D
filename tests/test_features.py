...
def test_as_row_follows_feature_names_exactly():
    vector = features.build(1, window(range(14)))
    row = vector.as_row()
...
def test_rising_and_falling_windows_have_opposite_slope():
    rising = features.build(1, window(range(14)))
    falling = features.build(1, window(range(13, -1, -1)))
...
def test_correlation_is_signed_and_ignores_series_too_short_to_measure():
    together = features.build(
        1, window(range(14)), other_sources={"youtube": window(range(14))}
    )
    opposed = features.build(
        1, window(range(14)), other_sources={"youtube": window(range(13, -1, -1))}
    )
    assert together.source_correlation > 0.9
    assert opposed.source_correlation < -0.9

    too_short = features.build(1, window(range(14)), other_sources={"youtube": window([1, 2])})
